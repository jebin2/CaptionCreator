...
def get_youtube_service(type='text'):
    ...
    if type == 'text' and youtube_service:
        logger_config.info("Using existing YouTube service.")
        return youtube_service
    
    if type == 'facts' and know_it_youtube_service:
        logger_config.info("Using existing KnowIt YouTube service.")
        return youtube_service
    ...
    logger_config.info("Checking for stored credentials...")
    creds = None
    if os.path.exists(specific_token):
        creds = Credentials.from_authorized_user_file(specific_token, SCOPES)
        logger_config.info("Found existing credentials.")

    # Refresh the token if it's expired
    if creds and creds.expired and creds.refresh_token:
        try:
            logger_config.info("Refreshing expired credentials...")
            creds.refresh(Request())  # This will use the refresh token to get a new access token
            logger_config.info("Credentials refreshed successfully.")
        except Exception as e:
            logger_config.error(f"Error refreshing token: {e}")
    ...
    if not creds or not creds.valid:
        if not creds or not creds.refresh_token:
            logger_config.info("No valid credentials or refresh token found. Initiating authentication flow.")
            flow = InstalledAppFlow.from_client_secrets_file(specific_client, SCOPES)
            creds = flow.run_local_server(port=0)
            logger_config.info("New credentials obtained successfully.")
        
        # Save the credentials to the file for the next run
        with open(specific_token, 'w') as token:
            token.write(creds.to_json())
            logger_config.info("Credentials saved to 'token.json'.")

    logger_config.info("Building YouTube service...")
    youtube_service = build('youtube', 'v3', credentials=creds)
    logger_config.info("YouTube service built successfully.")
    return youtube_service
...
def upload_video_to_youtube(video_path, thumbnail_path, title, description, type='text', riddle_shorts=False, old_video_id=None):
    ...
    logger_config.info(f"Starting upload for video: {video_path}")
...
"""
    ...
    logger_config.info(f"Uploading video: {video_path}")
    response = None
    try:
        while response is None:
            status, response = request.next_chunk()
            if status:
                logger_config.info(f'Uploaded {int(status.progress() * 100)}% of the video.')
    except Exception as e:
        logger_config.error(f"An error occurred during video upload: {e}")
        return False

    video_id = response['id']
    logger_config.info(f'Video uploaded successfully with ID: {video_id}')

    # Set the thumbnail (optional)
    try:
        logger_config.info(f"Uploading thumbnail: {thumbnail_path}")
        youtube.thumbnails().set(
            videoId=video_id,
            media_body=MediaFileUpload(thumbnail_path)
        ).execute()
        logger_config.info(f"Thumbnail uploaded successfully for video ID: {video_id}")
    except Exception as e:
        logger_config.error(f"An error occurred during thumbnail upload: {e}")
...
def process_entries_in_db(type):
    ...
    if entries:
        logger_config.info(f"Will upload after 1 hrs :: {entries}")
        return

    # Query for entries where generatedVideoPath and generatedThumbnailPath are not null
    logger_config.info("Fetching entries with videos and thumbnails ready for upload...")
    is_data_avail = False
    entries = None
    date_count = 0
    riddle_shorts = False
    ...
    for entry in entries:
        ...
        logger_config.info(f"Processing entry {entry_id}: {title}")

        # Upload the video to YouTube
        video_id = upload_video_to_youtube(video_path, thumbnail_path, title, description, type, riddle_shorts, video_id)
        
        if not video_id:
            logger_config.error(f"Error uploading video for entry {entry_id}. Stopping further uploads.")
            break  # Stop further uploads if there's an error

        # Mark the entry as uploaded to YouTube
        logger_config.info(f"Marking entry {entry_id} as uploaded to YouTube with video ID {video_id}.")
        ...
        logger_config.info(f"Entry {entry_id} successfully updated in the database.")

        logger_config.info(f"========================================================")
        logger_config.info(f"=                                                      =")
        logger_config.info(f"=Entry {entry_id} successfully posted to YT            =")
        logger_config.info(f"=                                                      =")
        logger_config.info(f"========================================================")


        logger_config.info("Sleeping for 1 minute before next upload...")
